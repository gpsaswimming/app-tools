...
/**
 * Merges an initial-seeding breakdown with a post-meet breakdown into per-team
 * seeded / added / billable counts, for the two-file comparison.
 *
 * Policy (swaps ignored): a metric's bill never drops below its seeded value and
 * grows only when the post-meet count exceeds it, i.e.
 *   billable = max(seeded, final),  added = max(0, final - seeded).
 * So scratches never reduce the bill, deck-added swims are charged, and a
 * like-for-like swap (a scratch offset by an add) is a wash.
 *
 * @param {ReturnType<typeof summarizeByTeam>} seededTeams  from the seeding file
 * @param {ReturnType<typeof summarizeByTeam>} finalTeams   from the post-meet file
 ...
 */
export function mergeTeamCounts(seededTeams, finalTeams) {
    const zero = { swimmers: 0, individualEntries: 0, relayEntries: 0 };
    const pick = (t) => ({ swimmers: t.swimmers, individualEntries: t.individualEntries, relayEntries: t.relayEntries });

    const byCode = new Map();
    for (const t of seededTeams) byCode.set(t.teamCode, { teamName: t.teamName, seeded: pick(t), final: { ...zero } });
    for (const t of finalTeams) {
        const rec = byCode.get(t.teamCode) || { teamName: t.teamName, seeded: { ...zero }, final: { ...zero } };
        rec.teamName = rec.teamName || t.teamName;
        rec.final = pick(t);
        byCode.set(t.teamCode, rec);
    }

    const metrics = ['swimmers', 'individualEntries', 'relayEntries'];
    return [...byCode.entries()]
        .map(([teamCode, rec]) => {
            const added = {}, billable = {};
            for (const m of metrics) {
                added[m] = Math.max(0, rec.final[m] - rec.seeded[m]);
                billable[m] = Math.max(rec.seeded[m], rec.final[m]);
            }
            return { teamCode, teamName: rec.teamName, seeded: rec.seeded, added, billable };
        })
        .sort((a, b) => a.teamName.localeCompare(b.teamName));
}
